export EncodingOp, lrEncodingOp, fourierEncodingOp, encodingOps_simple,
       encodingOps_parallel, encodingOp_multiEcho, encodingOp_multiEcho_parallel, encodingOps_expandedSignalModel
...
"""
    encodingOps_expandedSignalModel(acqData::AcquisitionData{T,D}, shape::NTuple{D,Int64}
                              , senseMaps::Array{Complex{T}}
                              ; kargs...) where {T,D}
...
* `senseMaps::Array{Complex{T}}`        - coil sensitivities
"""
function encodingOps_expandedSignalModel(acqData::AcquisitionData{T,D}, shape::NTuple{D,Int64}
                                , senseMaps::Array{Complex{T},4}
                                , higherOrderTrajectory::Matrix{T}
                                ; slice=1, kargs...) where {T,D}
  ...
  OffResonanceMap = kargs[:correctionMap]
  times = acqData.traj[1].times
  FieldOfView = acqData.fov 
 
  ESMOp = ExpandedSignalModelOp(smaps,OffResonanceMap,higherOrderTrajectory,shape,FieldOfView,times)
  
  # Higher order spherical Harmonics operator
