...
mutable struct InhomogeneityData{T}
  ...
  times::Vector{T}
  # AMM: If I want to be able to load 3D maps, I need to change this...
  # Cmap::Matrix{Complex{T}}
  Cmap::Array{Complex{T},3}
  t_hat::T
  z_hat::Complex{T}
  method::String
end
...
function FieldmapNFFTOp(shape::NTuple{D,Int64}, tr::Trajectory,
 ...
 # create and truncate low-rank expansion
  ...
  # AMM: From ComplexF64 - Float32
  # d = [zeros(ComplexF64, length(idx[κ])) for κ=1:K ]
  d = [zeros(ComplexF32, length(idx[κ])) for κ=1:K ]
