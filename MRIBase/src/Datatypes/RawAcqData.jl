...
function MRIBase.trajectory(f::RawAcquisitionData; slice::Int=1, contrast::Int=1)
  ...
  elseif (lowercase(f.params["trajectory"]) == "spiral")
    # AMM: adding spiral trajectory
    numRead = size(f.profiles[1].data,1).*numSet
    # traj = zeros(Float64, numRead, 1, 1, numRep)
    # times = zeros(Float64, numRead, 1, 1, numRep)
    traj = zeros(Float64, 3, numRead)
    times = zeros(Float64, 3, numRead)

    # traj_ = reshape(traj[:,:,:], D, :)
    tr = Trajectory(traj, size(traj,3), size(traj,2), circular=true, times=vec(times))
...
# AMM: adding sets function
sets(f::RawAcquisitionData) =
  [f.profiles[l].head.idx.set+1 for l=1:length(f.profiles)]
...
function rawdata(f::RawAcquisitionData; slice::Int=1, contrast::Int=1, repetition::Int=1)
  ...
  elseif lowercase(f.params["trajectory"]) == "spiral"
    # AMM: adding spiral
    numSampPerProfile = numSampPerProfile.*numSet
    kdata = zeros(typeof(f.profiles[1].data[1, 1]), numSampPerProfile, numChan)
    # posIdx = 1:Int(length(f.profiles)./numSet)
  ...
  # AMM: adding spiral
  if lowercase(f.params["trajectory"]) == "spiral"
    cnt = 1
    i_st = 1
    for l in posIdx # TODO: set of profiles (with unique encoding status)
      ...
      if f.profiles[l].head.idx.slice+1 != slice || f.profiles[l].head.idx.contrast+1 != contrast || f.profiles[l].head.idx.repetition+1 != repetition
        ...
        tmp =  Array{Float64}(undef, 0, 32)
        for i_set = i_st:i_st+numSet-1
          tmp = vcat(tmp,f.profiles[i_set].data)
        end
        kdata .= tmp
        cnt += 1
        i_st = l+numSet
