...
function reconstruction(acqData::AcquisitionData, recoParams::Dict)
  ...
  if recoParams[:reco] == "direct"
    ...
    elseif recoParams[:reco] == "expandedSignalModel"
      return reconstruction_expandedSignalModel(acqData, reconSize[1:encodingDims], reg, sparseTrafo, weights, L_inv, solvername, senseMaps, normalize, encOps, recoParams)
    else
