export reconstruction_simple, reconstruction_multiEcho, reconstruction_multiCoil, reconstruction_multiCoilMultiEcho, reconstruction_expandedSignalModel, reconstruction_lowRank, RecoParameters
...
"""
Performs a SENSE-type iterative image reconstruction including higher order field dynamics. Different slices and contrasts images
are reconstructed independently.
...
* (`params::Dict{Symbol,Any}`)          - Dict with additional parameters
"""
function reconstruction_expandedSignalModel(acqData::AcquisitionData{T}
                              , reconSize::NTuple{D,Int64}
                              , reg::Vector{Regularization}
                              , sparseTrafo
                              , weights::Vector{Vector{Complex{T}}}
  ...
  higherOrderTrajectory = params[:higherOrderTrajectory]

  # Repet weights
  # ToDo: Fix the way I repeat the weights for 3D data..
  # weights = repeat(weights[1],reconSize[3])
  ...
  for l = 1:1, k=1:1
    if encodingOps != nothing
      ...
      E = encodingOps_expandedSignalModel(acqData, reconSize, senseMapsUnCorr, higherOrderTrajectory; slice=k, encParams...)
    end
